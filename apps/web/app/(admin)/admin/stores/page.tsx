...
export default function AdminStoresPage() {
  ...
  const fetchStores = () => {
    ...
    apiFetch<ApiResponse>(`/admin/stores?${query.toString()}`)
      ...
      .catch((err: any) => {
        // Trước đây lỗi tải API bị nuốt và thay bằng 4 gian hàng GIẢ — admin
        // có thể tưởng nhầm là dữ liệu thật khi khoá/xoá/sửa gian hàng.
        setStores([]);
        setMsg({ text: err?.message || 'Không tải được danh sách gian hàng — vui lòng thử lại.', type: 'error' });
        setLoading(false);
      });
  };
